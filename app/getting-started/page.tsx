...
{/* Section 3: Networking Prerequisite */}
         <motion.section className="mt-8 sm:mt-12" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
           ...
           <div className="mt-4 sm:mt-5 space-y-1.5 sm:space-y-2">
             {osiLayers.map((layer) => (
               <button
                 ...
                 className={`w-full text-left cyber-card p-3 sm:p-3.5 flex items-start gap-3 sm:gap-4 transition-all ${activeLayer === layer.num ? 'border-cyber-cyan' : ''}`}
               >
                 <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-cyber-bg border border-cyber-border flex items-center justify-center flex-shrink-0">
                   <span className="text-[10px] sm:text-xs font-mono font-bold text-cyber-cyan">{layer.num}</span>
                 </div>
                 <div className="flex-1 min-w-0">
                   <div className="flex items-center gap-1.5 sm:gap-2">
                     <h4 className="text-xs sm:text-sm font-semibold text-white">{layer.name}</h4>
                     <span className="text-[9px] sm:text-[10px] px-1 sm:px-1.5 py-0.5 rounded bg-cyber-bg border border-cyber-border text-cyber-text-dim font-mono">
                       {layer.proto}
                     </span>
                   </div>
                   <p className="text-[10px] sm:text-xs text-cyber-text mt-0.5 sm:mt-1">{layer.desc}</p>
                   {activeLayer === layer.num && (
                     <motion.p
                       initial={{ opacity: 0, height: 0 }}
                       animate={{ opacity: 1, height: 'auto' }}
                       className="text-[10px] sm:text-xs text-cyber-amber mt-1.5 sm:mt-2 font-mono"
                     >
           ...
           <div className="mt-3 sm:mt-4 grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
             <div className="cyber-card p-3.5 sm:p-5">
               <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
                 <div className="w-7 h-7 sm:w-8 sm:h-8 rounded bg-cyber-green/10 flex items-center justify-center text-cyber-green">
                   <i className="ri-shield-check-line text-xs sm:text-sm" />
                 </div>
                 <h4 className="text-white font-semibold text-xs sm:text-sm">TCP (Transmission Control Protocol)</h4>
               </div>
               <ul className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-cyber-text">
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-green">+</span> Connection-oriented (three-way handshake)</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-green">+</span> Reliable delivery with acknowledgments</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-green">+</span> Ordered data delivery</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-red">-</span> Higher overhead and latency</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-red">-</span> Used by: HTTP, HTTPS, SSH, FTP, SMTP</li>
               </ul>
             </div>
             <div className="cyber-card p-3.5 sm:p-5">
               <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
                 <div className="w-7 h-7 sm:w-8 sm:h-8 rounded bg-cyber-amber/10 flex items-center justify-center text-cyber-amber">
                   <i className="ri-flashlight-line text-xs sm:text-sm" />
                 </div>
                 <h4 className="text-white font-semibold text-xs sm:text-sm">UDP (User Datagram Protocol)</h4>
               </div>
               <ul className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-cyber-text">
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-green">+</span> Connectionless — no handshake</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-green">+</span> Low overhead, fast transmission</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-green">+</span> Supports multicast and broadcast</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-red">-</span> No guaranteed delivery or ordering</li>
                 <li className="flex gap-1.5 sm:gap-2"><span className="text-cyber-red">-</span> Used by: DNS, DHCP, NTP, VoIP, gaming</li>
               </ul>
             </div>
           </div>
           ...
           <div className="mt-3 sm:mt-4 overflow-x-auto">
             <table className="w-full text-xs sm:text-sm border border-cyber-border rounded-lg overflow-hidden">
               <thead className="bg-cyber-bg-card">
                 <tr>
                   <th className="text-left px-2.5 sm:px-4 py-2 sm:py-3 text-cyber-text font-semibold">Port</th>
                   <th className="text-left px-2.5 sm:px-4 py-2 sm:py-3 text-cyber-text font-semibold">Protocol</th>
                   <th className="text-left px-2.5 sm:px-4 py-2 sm:py-3 text-cyber-text font-semibold">Service</th>
                 </tr>
               </thead>
               <tbody className="divide-y divide-cyber-border">
                 {commonPorts.map((p) => (
                   <tr key={p.port} className="hover:bg-cyber-bg-card/50 transition-colors">
                     <td className="px-2.5 sm:px-4 py-1.5 sm:py-2.5 font-mono text-cyber-cyan">{p.port}</td>
                     <td className="px-2.5 sm:px-4 py-1.5 sm:py-2.5 text-cyber-text">{p.proto}</td>
                     <td className="px-2.5 sm:px-4 py-1.5 sm:py-2.5 text-cyber-text">{p.service}</td>
                   </tr>
                 ))}
               </tbody>
             </table>
           ...
           <Callout type="tip" className="mt-3 sm:mt-4 text-xs sm:text-sm">
           ...
           <div className="mt-3 sm:mt-4 space-y-2.5 sm:space-y-3">
             <div className="cyber-card p-3 sm:p-4">
               <h4 className="text-xs sm:text-sm font-semibold text-white mb-1.5 sm:mb-2">CIDR Notation</h4>
...
/8   = 16,777,216     (massive network)`} />
             ...
             <div className="cyber-card p-3 sm:p-4">
               <h4 className="text-xs sm:text-sm font-semibold text-white mb-1.5 sm:mb-2">Private IP Ranges (RFC 1918)</h4>
               <p className="text-xs sm:text-sm text-cyber-text">
...
{/* Section 4: Common Terminology */}
          <motion.section className="mt-8 sm:mt-12" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            ...
            <motion.div className="mt-4 sm:mt-5 grid grid-cols-1 xs:grid-cols-2 gap-2.5 sm:gap-3" variants={fadeInStagger} initial="hidden" whileInView="visible" viewport={{ once: true }}>
             {terminology.map((t, i) => (
               <motion.button
                 key={i}
                 type="button"
